...
LABEL docker.image.base="mcr.microsoft.com/windows/servercore/iis" `
      ...
      docker.image.build="docker build --build-arg CERT_PFX_FILENAME='cert.pfx' --build-arg CERT_PFX_PASSWORD='PASSWORD' --tag DTR_SERVER/microsoft/netcore-runtime:3.1-iis ."

## Install dotnet 3.1.2 hosting pack
SHELL ["powershell", "-Command", "$ErrorActionPreference = 'Stop'; $ProgressPreference = 'Continue'; $verbosePreference='Continue';"]

ADD https://download.visualstudio.microsoft.com/download/pr/dd119832-dc46-4ccf-bc12-69e7bfa61b18/990843c6e0cbd97f9df68c94f6de6bb6/dotnet-hosting-3.1.2-win.exe "C:/setup/dotnet-hosting-3.1.2-win.exe"
RUN start-process -Filepath "C:/setup/dotnet-hosting-3.1.2-win.exe" -ArgumentList @('/install', '/quiet', '/norestart') -Wait
RUN Remove-Item -Force "C:/setup/dotnet-hosting-3.1.2-win.exe"
